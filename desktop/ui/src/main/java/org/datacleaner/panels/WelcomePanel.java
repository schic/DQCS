...
public class WelcomePanel extends DCSplashPanel {
	...
	public WelcomePanel(final AnalysisJobBuilderWindow window, final UserPreferences userPreferences,
		...
		_titleLabel = createTitleLabel(PropertyUtil.getProperty("datacleaner.ui.desktop.text.welcome"), false);
		...
		if (result == null) {
			...
			if (Version.isCommunityEdition()) {
				final JXEditorPane editorPane = new JXEditorPane("text/html",
						PropertyUtil.getProperty("datacleaner.ui.desktop.text.Description"));
	...
	private JComponent createButtonPanel() {
		final String newJobText = PropertyUtil.getProperty(SystemProperties.UI_DESKTOP_TEXT_NEW_JOB_BUTTON);
		...
		final PopupButton recentJobsButton =
				WidgetFactory.createDefaultPopupButton(PropertyUtil.getProperty("datacleaner.ui.desktop.text.button.recentjob"), IconUtils.FILE_HOME_FOLDER);
		recentJobsButton.setMenuPosition(MenuPosition.TOP);
		recentJobsButton.addActionListener(e -> refreshRecentJobs(recentJobsButton));
		final JButton browseJobsButton = WidgetFactory.createDefaultButton(PropertyUtil.getProperty("datacleaner.ui.desktop.text.button.browsejob"), IconUtils.FILE_FOLDER);
		browseJobsButton.addActionListener(_openAnalysisJobActionListener);

		final JButton manageDatastoresButton =
				WidgetFactory.createDefaultButton(PropertyUtil.getProperty("datacleaner.ui.desktop.text.button.managedatastores"), IconUtils.GENERIC_DATASTORE_IMAGEPATH);
