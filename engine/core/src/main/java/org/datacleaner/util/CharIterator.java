...
public class CharIterator implements ListIterator<Character> {
    ...
    public CharIterator(final CharSequence charSequence) {
    ...
    public CharIterator(final char[] chars) {
        if (chars == null) {
            _chars = new char[0];
        } else {
            _chars = chars;
    ...
    public CharIterator subIterator(final int fromIndex, final int toIndex) {
    ...
    public void remove() {
        ...
        final int numMoved = length() - _index - 1;
        if (numMoved > 0) {
