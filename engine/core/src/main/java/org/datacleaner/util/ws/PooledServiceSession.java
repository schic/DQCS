...
public class PooledServiceSession<R> extends RetryServiceSession<R> {
     ...
     * @param pool
     */
    public PooledServiceSession(final GenericObjectPool<Integer> pool) {
        this(pool, 0, null);
    }
     ...
     */
    public PooledServiceSession(final int maxConnections) {
     ...
     * @param sleepTimeBetweenRetries
     */
    public PooledServiceSession(final int maxConnections, final int maxRetries, final int[] sleepTimeBetweenRetries) {
        super(maxRetries, sleepTimeBetweenRetries);
    ...
    private void returnObject(final Integer poolObject) {
        try {
