...
final class PollingBasedFileMonitor implements FileMonitor {
    ...
    public PollingBasedFileMonitor(final File file) {
        _file = file;
     ...
     */
    private synchronized boolean poll(final long currentTimeMillis) {
        final long timeDiffMillis = currentTimeMillis - _lastPolled;
